...
from fpl_dashboard.data.fpl_api_handler import FPLAPIEndpoint, FPLAPIHandler
...
def get_top_players_by_pos_and_metric(pos: list, metric: str):
    query = (
        select(Element)
        .where(Element.player_position.has(ElementType.singular_name_short.in_(pos)))
        .order_by(getattr(Element, metric).desc())
        .limit(10)
    )
...
def players_adjusted_ownership(positions):
    players = session.exec(
        select(Element).where(
            Element.player_position.has(ElementType.singular_name_short.in_(positions))
        )
    ).all()
    mu = sum([player.total_points for player in players]) / len(players)
    sigma = sum([((player.total_points - mu) ** 2) for player in players]) / len(
        players
    )
    index = [
        (
            player,
            round(
                (player.total_points - mu)
                / sigma
                * (1 - player.selected_by_percent / 100)
                * player.total_points,
                2,
            ),
        )
...
def players_adjusted_consistency(positions):
    ...
    return sorted(
        [
            (player, round(player.points_adjusted_for_consistency, 2))
            for player in players
        ],
        key=lambda x: x[1],
        reverse=True,
    ...
    return sorted(
        [
            (
                player,
                round(
                    player.points_adjusted_for_consistency
                    * (1 - player.selected_by_percent / 100),
                    2,
                ),
            )
    ...
    with session:
        ...
        for pos in positions:
            ...
            for player in top_players:
                print(
                    player.player_position.singular_name_short,
                    player.player_team.short_name,
                    player.total_points,
                    player.web_name,
                )
            ...
            for player in players_adjusted_ownership([pos]):
                print(
                    player[0].player_position.singular_name_short,
                    player[0].player_team.short_name,
                    player[0].total_points,
                    player[0].web_name,
                    player[1],
                )
